    ...
    for y, row in enumerate(board):
        ...
        for piece in row:
            if piece == Piece.BLACK:
                line += f"\033[1;30mB\033[0m  "
            elif piece == Piece.WHITE:
                line += f"\033[1;37mW\033[0m  "
            else:
                line += ".  "
        print(line)
    print()
...
def main():
    ...
    while not game.is_terminal():
        ...
        print(
            f"Turn {turn}: Player {'BLACK' if game.current_player == Piece.BLACK else 'WHITE'} ({agent_type}) moves from {move.from_pos} to {move.to_pos}")
